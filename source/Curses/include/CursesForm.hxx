...
class CursesForm {
    public:
        CursesForm(const size_t& number_of_fields);
        ~CursesForm();

        void initForm();
        void setField(size_t field_index, int height, int width, int toprow, 
                      int leftcol, int offscreen, int nbuffers);
        void initField(const size_t& number_of_fields);
    ...
    private:
        ...
        std::vector<std::string> field_buffers;
};
